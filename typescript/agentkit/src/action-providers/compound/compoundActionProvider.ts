...
- The token must be an approved collateral asset for the Compound market
  ...
  async supply(
    ...
    try {
      ...
      const amountAtomic = formatAmountWithDecimals(args.amount, decimals);

      // Check wallet balance before proceeding
      const walletBalance = await getTokenBalance(wallet, tokenAddress);
      if (walletBalance < amountAtomic) {
        const humanBalance = formatAmountFromDecimals(walletBalance, decimals);
  ...
  async withdraw(
    ...
    try {
      ...
      if (amountAtomic > collateralBalance) {
        const humanBalance = formatAmountFromDecimals(collateralBalance, decimals);
...
- Ensure you have sufficient collateral to borrow
  ...
  async borrow(
    ...
    try {
      ...
      // USDC has 6 decimals
      const amountAtomic = formatAmountWithDecimals(args.amount, 6);
      ...
      const baseTokenAddress = (await wallet.readContract({
...
- Ensure you have sufficient balance of the asset to repay
  ...
  async repay(
    ...
    try {
      ...
      const amountAtomic = formatAmountWithDecimals(args.amount, tokenDecimals);
      const tokenBalance = await getTokenBalance(wallet, tokenAddress);

      if (tokenBalance < amountAtomic) {
        const humanBalance = formatAmountFromDecimals(tokenBalance, tokenDecimals);
