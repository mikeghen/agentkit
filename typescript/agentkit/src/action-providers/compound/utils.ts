...
/**
 * Format amount from decimals to human readable
 *
 * @param amount - The amount to format
 * @param decimals - The number of decimals
 * @returns The formatted amount as a string
 */
export const formatAmountFromDecimals = (amount: bigint, decimals: number): string => {
  return formatUnits(amount, decimals);
};

/**
 * Format amount with decimals from human readable
 *
 * @param amount - The amount to format
 * @param decimals - The number of decimals
 * @returns The formatted amount as a bigint
 */
export const formatAmountWithDecimals = (amount: string, decimals: number): bigint => {
...
export const getHealthRatio = async (
  ...
  if (!cometAddress) {
    throw new Error("Comet address is undefined");
  }
...
export const getHealthRatioAfterWithdraw = async (
  ...
  for (const supply of supplyDetails) {
    ...
    if (supplyTokenSymbol === withdrawTokenSymbol) {
      ...
      const withdrawAmountHuman = new Decimal(formatAmountFromDecimals(amount, decimals));
...
export const getHealthRatioAfterBorrow = async (
  ...
  const additionalBorrow = new Decimal(formatAmountFromDecimals(amount, baseDecimals));
...
const getPriceFeedData = async (
  ...
  if (!priceFeedAddress) {
    throw new Error("Price feed address is undefined");
  }
...
const getBorrowDetails = async (
  ...
  const humanBorrowAmount = new Decimal(formatAmountFromDecimals(borrowAmountRaw, baseDecimals));
...
> => {
  ...
  for (let i = 0; i < numAssets; i++) {
    ...
    if (collateralBalance > 0n) {
      ...
      const humanSupplyAmount = new Decimal(formatAmountFromDecimals(collateralBalance, decimals));
 ...
 */
export const getCometAddress = (network: Network): `0x${string}` => {
 ...
 */
export const getAssetAddress = (network: Network, assetId: string): `0x${string}` => {
  if (!network.networkId) {
    throw new Error("Network ID is required");
  }
