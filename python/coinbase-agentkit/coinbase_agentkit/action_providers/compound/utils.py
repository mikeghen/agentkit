...
def get_health_ratio_after_borrow(wallet: EvmWalletProvider, compound_address: str, borrow_amount: str) -> Decimal:
    ...
    return Decimal("Infinity") if new_borrow_value == 0 else total_adjusted_collateral / new_borrow_value
...
def get_health_ratio_after_withdraw(
    ...
    for supply in supply_details:
        if supply["Token Symbol"] == get_token_symbol(wallet, asset_address):
            ...
            decimals = get_token_decimals(wallet, asset_address)
            withdraw_amount_human = Decimal(format_amount_from_decimals(int(withdraw_amount), decimals))
